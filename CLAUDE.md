...
5. **Recharts + Next.js SSR crash**: Recharts uses browser APIs (window, document) that don't exist during SSR. Even with `"use client"`, Next.js 16 pre-renders on the server. ClientOnly wrappers help but Vercel's `framework: "nextjs"` still runs server rendering. **Final fix**: use `output: "export"` for static HTML and remove `framework` from `vercel.json` entirely. Vercel then serves plain static files with zero SSR.
